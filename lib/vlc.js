...
module.exports = (function() {
    ...
    if (!playing) {
      ...
      if (id == 'youtube') {
        ...
        if(url.indexOf("?list=") != -1) {
          ...
          var child = spawn("youtube-dl", ["-j", "--flat-playlist", url]);
          var list = "";
          child.stdout.on('data', function(data) {
            list += data;
          });
          child.on('close', function(code) {
            var links = list.split('\n');
            links.pop(); //The last element is an empty string
            for(var i=0; i<links.length; i++) {
              link = JSON.parse(links[i]);
              console.log("Adding song: " + link['title']);
              play("http://www.youtube.com/watch?v=" + link['url'], "youtube");
            }
          })
        }
        else if (url.indexOf('?v=') != -1 || url.indexOf('&v=') != -1) {
  ...
  var volume = function(change) {
    if(change == '0') {
      ...
      var child = spawn("nircmd.exe", ["changesysvolume", change]);
    }
  }
